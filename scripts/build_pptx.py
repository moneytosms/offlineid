...
Design: professional navy + sky-blue corporate look. 16:9, 16 slides.
...
# Content slides: white background, navy + blue accents
BG        = C("FFFFFF")   # slide background
SURFACE   = C("F1F5F9")   # panel / card background
SURFACE2  = C("EAF1FB")   # alternating row tint
LINE      = C("CBD5E1")   # subtle border
LINEB     = C("93C5FD")   # blue-tinted border
NAVY      = C("1E3A5F")   # deep navy  (spine, header band, dark slides)
ACCENT    = C("2563EB")   # primary blue
ACCENTDIM = C("DBEAFE")   # light blue (table header fill)
WARN      = C("D97706")   # amber / orange
DANGER    = C("DC2626")   # red
INFO      = C("0891B2")   # teal / sky
TEXT      = C("0F172A")   # near-black body text
TEXTDIM   = C("475569")   # medium slate
TEXTFAINT = C("94A3B8")   # light slate
ONDK      = C("F0F7FF")   # text on dark (navy) slides
...
def slide(dark=False):
    ...
    s.background.fill.fore_color.rgb = NAVY if dark else BG
    return s
...
def chrome(s, idx, kicker=None, title=None):
    """Header chrome for content slides (white background)."""
    # left navy spine
    rect(s, 0, 0, 0.12, SH, fill=NAVY)
    # top navy band
    rect(s, 0.12, 0, SW - 0.12, 0.08, fill=NAVY)

    # footer
    hline(s, 0.9, 6.86, 11.5, color=LINE)
    rect(s, 0.9, 6.97, 0.06, 0.15, fill=NAVY)
    text(s, 1.08, 6.94, 8.5, 0.4,
         [[R("OFFLINE·ID", 9, TEXTFAINT, True, MONO),
           R("   ·   NHAI HACKATHON 7.0   ·   DATALAKE 3.0 INTEGRATION MODULE", 9, TEXTFAINT, False, MONO)]])
    text(s, SW - 1.9, 6.94, 1.1, 0.4,
         [[R(f"{idx:02d} / {NSLIDES}", 9, TEXTFAINT, False, MONO)]], align=PP_ALIGN.RIGHT)

    y = 0.52
    if kicker:
        text(s, 0.9, y, 11.3, 0.36,
             [[R(kicker, 11, ACCENT, True, MONO)]])
        y += 0.4
    if title:
        text(s, 0.88, y, 11.6, 1.1, [[R(title, 28, NAVY, True, TITLE_FONT)]])
        rect(s, 0.92, y + 0.7, 0.55, 0.05, fill=ACCENT)

    hline(s, 0.9, 1.78, 11.5, color=LINE)
    return 1.96  # content top y


def card(s, x, y, w, h, border=LINE, fill=SURFACE):
    return rect(s, x, y, w, h, fill=fill, line=border, line_w=1.5,
                shape=MSO_SHAPE.ROUNDED_RECTANGLE)


def card_label(s, x, y, w, label, color):
    rect(s, x + 0.22, y + 0.2, 0.28, 0.06, fill=color)
    text(s, x + 0.22, y + 0.34, w - 0.44, 0.4, [[R(label, 13, color, True, MONO)]])


def chip(s, x, y, label, color=ACCENT, w=2.0):
    c = rect(s, x, y, w, 0.44, fill=None, line=color, line_w=1.5,
             shape=MSO_SHAPE.ROUNDED_RECTANGLE)
    tf = c.text_frame
    tf.margin_top = 0
    tf.margin_bottom = 0
...
def chip_filled(s, x, y, label, color=ACCENT, w=2.0):
    """Solid filled chip — for dark slides."""
    c = rect(s, x, y, w, 0.44, fill=color, line=None,
             shape=MSO_SHAPE.ROUNDED_RECTANGLE)
    tf = c.text_frame
    tf.margin_top = 0
    tf.margin_bottom = 0
...
def flow_box(s, x, y, w, h, title, sub, accent=ACCENT):
    ...
    rect(s, x, y + 0.1, 0.06, h - 0.2, fill=accent)
    text(s, x + 0.24, y + 0.08, w - 0.44, h - 0.16,
         [[R(title, 14, NAVY, True, TITLE_FONT)],
...
rect(s, 0, 0, SW, 0.1, fill=ACCENT)
rect(s, 0, SH - 0.1, SW, 0.1, fill=ACCENT)
rect(s, 0, 0, 0.1, SH, fill=ACCENT)
rect(s, SW - 0.1, 0, 0.1, SH, fill=ACCENT)

rect(s, 0.9, 1.42, 0.06, 0.32, fill=ACCENT)
text(s, 1.08, 1.45, 11.5, 0.4,
     [[R("BIOMETRIC FIELD TERMINAL", 13, ACCENT, True, MONO)]])
text(s, 0.85, 2.0, 12, 1.5,
     [[R("OFFLINE·ID", 72, ONDK, True, TITLE_FONT)]])
rect(s, 0.92, 3.4, 2.6, 0.06, fill=ACCENT)
text(s, 0.9, 3.66, 11.0, 1.2,
     [[R("Secure offline facial recognition & liveness detection", 21, ONDK, False, TITLE_FONT)],
      [R("for field personnel in zero-network zones.", 21, ONDK, False, TITLE_FONT)]],
     space_after=2)
chip(s, 0.9, 5.18, "100% ON-DEVICE", ACCENT, 2.4)
chip(s, 3.46, 5.18, "ANDROID + iOS", INFO, 2.1)
chip(s, 5.72, 5.18, "9.1 MB MODELS", WARN, 2.2)
chip(s, 8.08, 5.18, "OPEN SOURCE", TEXTFAINT, 2.0)
hline(s, 0.92, 6.3, 3.5, color=C("334155"))
text(s, 0.9, 6.48, 11, 0.55,
     [[R("NHAI Hackathon 7.0", 13, ONDK, True, TITLE_FONT),
       R("   ·   Datalake 3.0 integration module   ·   v1.4.0", 13, TEXTFAINT, False, TITLE_FONT)]])
...
bullets(s, 0.9, top, 11.6, [
    ...
    ("Constrained hardware.", "Mid-range phones: 3 GB RAM, Android 8+ / iOS 12+, no GPU."),
...
card(s, 0.9, top + 3.52, 11.5, 0.98, border=LINEB, fill=SURFACE2)
rect(s, 0.9, top + 3.52, 0.06, 0.98, fill=ACCENT)
text(s, 1.18, top + 3.66, 11.0, 0.72,
...
text(s, 0.9, top, 11.6, 1.1,
     [[R("Detect → prove liveness → recognise in ", 18, TEXT, False, TITLE_FONT),
       ...
       R(", no internet, then ", 18, TEXT, False, TITLE_FONT),
       R("sync-and-purge", 18, NAVY, True, TITLE_FONT),
...
cards_data = [
    ...
    ("ENCRYPTED", "AES-256-GCM faceprints;\nno raw images stored.", NAVY),
    ("SYNC & PURGE", "Presigned S3 upload, then\nlocal records deleted.", WARN),
]
cw = 2.78
gapx = 0.13
x0 = 0.9
for i, (h, b, col) in enumerate(cards_data):
    x = x0 + i * (cw + gapx)
    card(s, x, top + 1.42, cw, 2.58, border=col, fill=SURFACE)
    rect(s, x + 0.22, top + 1.64, 0.26, 0.06, fill=col)
    text(s, x + 0.22, top + 1.78, cw - 0.4, 2.0,
         [[R(h, 14, col, True, MONO)],
...
steps = [
    ("1 · Detect", "SCRFD-500M → bounding box + 5 landmarks", ACCENT),
    ("2 · Passive liveness", "FASNet ×2 (scale 2.7 / 4.0) anti-spoof", INFO),
    ("3 · Active liveness", "ML Kit gesture: blink / smile / turn", INFO),
    ("4 · Recognise", "ArcFace align → MobileFaceNet INT8 → 512-d", ACCENT),
    ("5 · Match & log", "cosine vs enrolled → encrypted attendance row", NAVY),
    ("6 · Sync & purge", "reconnect → presigned S3 PUT → local delete", WARN),
]
bx = 0.9
bw = 11.5
bh = 0.56
...
for i, (big, small, body) in enumerate(trip):
    ...
    rect(s, x + 0.25, top + 0.42, 0.44, 0.06, fill=ACCENT)
    text(s, x + 0.25, top + 0.56, cw - 0.5, 1.3,
         [[R(big, 38, NAVY, True, TITLE_FONT)],
          [R(small.upper(), 12, ACCENT, True, MONO)]], space_after=4)
    text(s, x + 0.25, top + 2.02, cw - 0.5, 1.5,
...
text(s, 0.9, top + 4.05, 11.5, 0.6,
     [[R("All MIT-licensed. ", 14, NAVY, True, TITLE_FONT),
       R("Models 9.1 MB + ONNX Runtime ≈ 12.6 MB added to the Datalake APK, "
         "well inside the 20 MB brief budget.", 14, TEXTDIM, False, TITLE_FONT)]],
     line_spacing=1.1)
...
card(s, 0.9, top + 3.92, 11.5, 0.64, border=LINEB, fill=SURFACE2)
text(s, 1.2, top + 3.92, 11.0, 0.64,
     [[R("GATE   ", 14, NAVY, True, MONO),
...
bullets(s, 0.9, top + 0.1, 11.6, [
    ("Encrypted at rest.", "Faceprints sealed with AES-256-GCM; key in Android Keystore."),
    ("No raw images.", "Only 512-d embeddings stored, never photos of faces."),
    ...
    ("Purge on sync.", "Attendance rows deleted locally once upload is confirmed."),
], fs=16, gap=14)
...
card(s, 0.9, top + 0.12, 5.65, 3.45, border=LINE, fill=SURFACE)
card_label(s, 0.9, top + 0.12, 5.65, "ON DEVICE", NAVY)
...
card(s, 6.75, top + 0.12, 5.65, 3.45, border=LINE, fill=SURFACE)
...
text(s, 0.9, top + 3.76, 11.5, 0.4,
     [[R("Devices stay lean; the server becomes the single source of truth.", 13.5, TEXTFAINT, False, TITLE_FONT)]])
...
for i, (big, small, tgt) in enumerate(metrics):
    ...
    text(s, x + 0.22, top + 0.28, cw - 0.4, 1.6,
         [[R(big, 30, NAVY, True, TITLE_FONT)],
          [R(small.upper(), 11, ACCENT, True, MONO)],
          [R(tgt, 11, TEXTFAINT, False, MONO)]], space_after=3)
table(s, 0.9, top + 2.28, 11.5,
      ["Model", "Size", "Avg", "P95"],
...
text(s, 0.9, top + 4.18, 11.6, 0.8,
     [[R("Host CPU · ONNX Runtime.  Pipeline = detect + 2× liveness + embed; "
         "align & cosine < 10 ms for end-to-end.", 12, TEXTDIM, False, TITLE_FONT)],
...
layers = [
    ...
    ("Native engine · Kotlin (Android) / Swift (iOS)", "ONNX Runtime: detect · liveness · embed", NAVY),
...
# ======================================================================================
#  SLIDE 12 — FEASIBILITY / INTEGRATION
# ======================================================================================
s = slide()
...
card(s, 0.9, top + 3.48, 11.5, 1.02, border=LINEB, fill=SURFACE2)
rect(s, 0.9, top + 3.48, 0.06, 1.02, fill=ACCENT)
text(s, 1.18, top + 3.6, 11.0, 0.82,
...
bullets(s, 0.9, top + 0.1, 11.6, [
    ("Open-source only.", "MIT / Apache stack, no paid licences, full source shared."),
...
bullets(s, 0.9, top + 0.1, 11.6, [
    ...
    ("Lean by design.", "SQLite queue → batch S3 → purge keeps device storage flat."),
    ("Swappable models.", "update accuracy by replacing ONNX files; 8 MB headroom remains."),
    ("Demographic tuning.", "on-device fine-tune on an Indian face subset (roadmap)."),
    ("iOS device build + GPU delegate.", "next step on the path to full parity."),
], fs=16, gap=13)
...
# ======================================================================================
#  SLIDE 16 — CLOSING  (dark navy background)
# ======================================================================================
s = slide(dark=True)
rect(s, 0, 0, SW, 0.1, fill=ACCENT)
rect(s, 0, SH - 0.1, SW, 0.1, fill=ACCENT)
...
rect(s, 0.9, 1.62, 0.06, 0.32, fill=ACCENT)
text(s, 1.08, 1.65, 11.5, 0.4,
     [[R("OFFLINE·ID   ·   v1.4.0", 15, ACCENT, True, MONO)]])
text(s, 0.85, 2.18, 11.6, 1.5,
     [[R("Authenticate anyone, anywhere,", 34, ONDK, True, TITLE_FONT)],
      [R("zero network required.", 34, ONDK, True, TITLE_FONT)]], space_after=2)
rect(s, 0.92, 3.68, 2.2, 0.06, fill=ACCENT)
text(s, 0.9, 3.98, 11.5, 1.2,
     [[R("Secure · lightweight · fully offline face auth that plugs into Datalake 3.0.",
         16, ONDK, False, TITLE_FONT)]])
text(s, 0.9, 5.02, 11.5, 1.2,
     [[R("Source     ", 13, TEXTFAINT, True, MONO),
       R("github.com/moneytosms/offlineid", 13, ACCENT, False, MONO)],
      [R("Release    ", 13, TEXTFAINT, True, MONO),
       R("github.com/moneytosms/offlineid/releases/tag/v1.4.0", 13, ACCENT, False, MONO)]],
     space_after=6, line_spacing=1.2)
text(s, 0.9, 6.44, 11.5, 0.5,
     [[R("Thank you   ·   Q & A", 15, ONDK, True, TITLE_FONT)]])
